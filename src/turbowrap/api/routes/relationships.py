...
def check_existing_link(db: Session, source_id: str, target_id: str) -> bool:
    ...
    existing = db.query(RepositoryLink).filter(
        ((RepositoryLink.source_repo_id == source_id) & (RepositoryLink.target_repo_id == target_id)) |
        ((RepositoryLink.source_repo_id == target_id) & (RepositoryLink.target_repo_id == source_id))
    ).first()
...
async def analyze_relationships(
    ...
    if len(repos) < 2:
        raise HTTPException(
            status_code=400,
            detail="Servono almeno 2 repository per analizzare le relazioni"
        )
    ...
    for repo in repos:
        ...
        if content:
            repo_structures.append(RepoStructure(
                id=repo.id,
                name=repo.name,
    ...
    if len(repo_structures) < 2:
        raise HTTPException(
            status_code=400,
            detail="Servono almeno 2 repository con STRUCTURE.md per l'analisi"
        )
    ...
    try:
        ...
        try:
            ...
            if json_match:
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"Errore parsing risposta AI: {str(e)}"
                )

        connections = []
        for conn in result.get("connections", []):
            connections.append(IdentifiedConnection(
